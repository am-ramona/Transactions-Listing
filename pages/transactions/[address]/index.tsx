...
export default function Transactions() {
    ...
    async function getEthereumData(order: boolean) {
        ...
        try {
            const data = await alchemy.forNetwork(network).core.getAssetTransfers({
                fromBlock: "0x0",
                fromAddress: address,
            ...
            const balance = await alchemy.forNetwork(network).core.getTokenBalances(
                address,
    ...
    return (
        <section className={styles.frame}>
                    ...
                    {(address
                        ...
                        && data.transfers.map((item, index) =>
                            <div key={index} className={styles.grid}>
                                ...
                                <Link href={{ pathname: `${address}/${item.hash}`, query: { amount: item.value, timestamp: item.metadata.blockTimestamp, block_explorer: block_explorer } }}> see more </Link>
                            </div>
                        )}
                </div>
            ...
            <main>
                <p> Address current balance is: {balance.tokenBalances && balance.tokenBalances[0].tokenBalance}</p>
                <p> Sort by <span className={`${styles.red} ${styles.pointer}`} onClick={() => getEthereumData(!order)}>Timestamp</span></p>
            </main>
        </section>
    )
}